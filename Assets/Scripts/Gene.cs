...
public class Gene : MonoBehaviour {
	...
	public static int Equals (Gene currentGene, Gene newGene){
		if (newGene.RpsValue == Gene.RPS.Zero) {
		...
		} else if (currentGene.RpsValue == newGene.RpsValue) { //both gene types are the same; their strengths are compared.
			...
			} else { //the new gene is small
				return -1; //the new gene is small and the old gene is medium or big; the old gene is stronger.
			}
		} else { //gene types are different; the strengths are ignored and the types are compred
